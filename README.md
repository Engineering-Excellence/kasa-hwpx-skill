...
`--pad-hour`를 붙이면 KASA 표기법(시 두 자리)을 **적용 범위 안에서만** 함께 교정합니다
(`9:20`→`09:20`). 순연 없이 표기만 고치려면 `--shift "+0m" --pad-hour`.

숫자 일부만 글자 서식이 달라 한 시각이 두 `<hp:t>`에 쪼개져 있으면(`’26년 10월 7일, 12`
+ `:30 예정`) 노드 단위 탐색으로는 순연되지 않습니다. 이런 지점은 **드라이런·실제 실행
양쪽에서 경고 블록**으로 알리고, `--split-fix-map 분리시각.json`으로 `redraft.py --mode
exact`에 그대로 넣을 매핑을 만들어 줍니다(문서 안에서 유일한 텍스트만 담습니다).
자동 병합은 글자 모양을 바꿀 수 있어 하지 않으며, CI에서는 `--fail-on-split`으로
탐지 시 파일을 쓰지 않고 중단시킬 수 있습니다.
...
치환은 **원본 기준 동시 적용**이라, 시간표를 30분씩 순연하는 매핑처럼 새 값이 다른 항목의
기존 값과 겹쳐도 연쇄 치환(도미노)이 일어나지 않습니다(겹치는 키는 긴 쪽이 먼저 매칭).
...
제외·자정 넘김·자기 검산·표기 교정)·분리 노드 시각 탐지(결합 경계·범위 필터·수정 매핑
유일성)를 잠근다(171케이스).
...
- 재기안 치환은 단일 정규식 alternation으로 **원본을 1회 스캔**해 동시 적용 — 규칙 간
  연쇄 치환이 없고(시각 순연 매핑도 안전), 겹치는 키는 긴 쪽이 먼저 매칭.
...
**현재 버전: `v0.11.0`** (버전의 단일 출처는 `kasa-hwpx/SKILL.md` 프런트매터의 `version` 필드이며,
...
- `v0.10.0` feat: 시각 순연에 표기 교정 옵션(`--pad-hour`) — KASA 표기법(시 두 자리)을
  **적용 범위 안에서만** 함께 교정(`9:20`→`09:20`)해 보존 구간의 표기는 건드리지 않는다.
  ...
  fix: `--shift "-1:00"`을 argparse가 옵션으로 오인하던 문제. 회귀 테스트 153케이스
- `v0.11.0` feat: 글자 서식으로 분리된 시각 탐지·경고(`shift_time.py`) — 한 시각이 서로 다른
  charPr의 두 `<hp:t>`에 걸쳐 있어 **조용히 순연에서 빠지던** 지점을 드라이런·실제 실행
  양쪽에서 경고한다(실측 누락 1건 재현). 문단·표 셀·줄바꿈 경계는 결합하지 않아 오탐이 적고,
  `--split-fix-map`으로 `redraft --mode exact`용 매핑 생성(문서 내 유일 텍스트만),
  `--fail-on-split`으로 CI 차단. 자동 병합은 하지 않는다. 회귀 테스트 171케이스 **(현재)**
