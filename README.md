...
표기법 lint는 공문서 표기 원칙을 KASA 보고서에 맞게 점검합니다 —
날짜 `2026. 7. 7.`(온점+공백, 앞 0 제거, `’YY.MM.DD.` 축약 허용), 시간 24시각제 쌍점,
4자리 이상 숫자의 천 단위 쉼표(연도 제외), 항목 기호 □/ㅇ/-/※/* 위계(유사 기호·위계 역전 탐지),
물결표(~) 붙여쓰기.
...
이미지 삽입/교체/삭제(BinData·manifest·pic 일관성, MI 보호)·secure-fill(값 비노출)·
PreToolUse 가드 훅(차단/통과 조건)·office unpack→repack 왕복 바이트 보존을 잠근다(94케이스).
...
**현재 버전: `v0.7.0-rc1`** (버전의 단일 출처는 `kasa-hwpx/SKILL.md` 프런트매터의 `version` 필드이며,
...
- `v0.7.0-rc1` build: 업스트림 반영 3차 사전 검증 빌드 **(현재, 한글 실물 열람 검증 통과 — 정식 v0.7.0 릴리스 대기)**
  — KASA 표기법 lint(`kasa_lint.py`, validate 통합), Claude Code PreToolUse 가드 훅 3종(`hooks/`),
  직인/서명 이미지 삽입·교체·삭제(`hwpx_image.py`), unpack→repack 왕복 바이트 보존 회귀 잠금(총 94케이스).
