...
try:
    import kasa_lint as _lint
except ImportError:  # validate.py 단독 복사 시 표기법 점검만 생략
    _lint = None
...
def kasa_check(path):
    ...
    # 표기법 lint(날짜·시간·숫자·항목 기호·위계) — kasa_lint 규칙 통합
    if _lint is not None:
        warns = _lint.lint_paragraphs(_lint.paragraphs_from_parts(parts))
        if warns:
            notes.extend(warns)
        else:
            notes.append("[OK] 표기법(날짜·시간·숫자·항목 기호·위계) 위반 없음")
    return notes
