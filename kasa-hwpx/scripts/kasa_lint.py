...
점검 규칙:
  날짜   2026. 7. 7. 형식(온점+공백, 앞 0 제거, 일 뒤 온점). ’YY.MM.DD. 축약형은 허용.
         요일은 날짜에 붙여 씀: 2026. 7. 7.(화) — '7. (화)'처럼 띄면 경고.
  시간   24시각제 쌍점 표기(14:30). '오후 2시'·'14시 30분'은 경고.
         시·분은 두 자리(날짜와 반대): 8:9 → 08:09.
  숫자   4자리 이상 정수의 천 단위 쉼표(연도 제외).
  기호   항목 기호는 □/ㅇ/-/※/* 위계만 사용(○·●·■·• 등 유사 기호 경고).
  위계   ㅇ 항목이 □ 제목보다, - 항목이 ㅇ 항목보다 먼저 나오면 경고.
  기간   물결표(~·∼·〜)는 앞뒤 붙여 씀.
  쌍점   쌍점 뒤는 한 칸 띄움: '원장:김갑동' → '원장: 김갑동'.
(근거: 행정업무운영 편람 — chrisryugj/kordoc gongmunseo-reference.md와 대조 검증)
...
# 시각 두 자리(08:09) — 시나 분이 한 자리면 경고(비율 표기일 수 있어 문구로 안내)
_TIME_PAD = re.compile(r"(?<![\d:])(\d{1,2}):(\d{1,2})(?![\d:])")
# 쌍점 뒤 붙여 쓴 한글/영문(시각 HH:MM은 숫자라 걸리지 않음)
#   ※ '앞말과 쌍점 붙임' 검사는 기준 양식 원문에 '편집용지 : 좌…' 표기가 있어 제외
_COLON_TIGHT = re.compile(r"[가-힣][::](?=[가-힣A-Za-z])")
# 요일 괄호는 날짜에 붙여 씀: '7. 7.(화)' — '7. 7. (화)'처럼 띄면 경고
_WEEKDAY_GAP = re.compile(r"\d\.\s+\(([월화수목금토일])\)")
    ...
    for txt in paras:
        ...
        for m in _TILDE.finditer(txt):
            warns.append(f"[경고] 표기법(기간): '{m.group().strip()}' — "
                         f"물결표(~)는 앞뒤 붙여 씀")
        for m in _TIME_PAD.finditer(txt):
            h, mi = m.groups()
            if len(h) == 2 and len(mi) == 2:
                continue
            warns.append(f"[경고] 표기법(시간): '{m.group()}' — 시각이라면 시·분을 "
                         f"두 자리로(예: {int(h):02d}:{int(mi):02d}, 날짜와 반대)")
        for m in _COLON_TIGHT.finditer(txt):
            warns.append(f"[경고] 표기법(쌍점): '{m.group()}…' — 쌍점 뒤는 "
                         f"한 칸 띄움(예: 원장: 김갑동)")
        for m in _WEEKDAY_GAP.finditer(txt):
            warns.append(f"[경고] 표기법(날짜): '{m.group()}' — 요일 괄호는 "
                         f"날짜에 붙여 씀(예: 2026. 7. 7.(화))")
