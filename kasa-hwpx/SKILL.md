...
요청
 ...
 ├─ "발사가 밀렸어/일정 N시간 순연"     → 워크플로우 B-4 (shift_time, 범위 지정 순연)
...
> ```
>
> **치환은 원본 기준 동시 적용이다.** 모든 키를 한 번에 스캔해 바꾸므로, 새 값이 다른
> 항목의 기존 값과 겹쳐도 연쇄 치환(도미노)이 일어나지 않는다. 시간표를 30분씩 순연하는
> 매핑을 그대로 넣어도 안전하다(겹치는 키는 긴 쪽이 먼저 매칭된다):
> ```json
> {"09:00~09:30": "09:30~10:00", "09:30~10:00": "10:00~10:30",
>  "10:00~11:00": "10:30~11:30", "11:00~12:00": "11:30~12:30"}
> ```
> → 각 행이 정확히 한 번씩만 이동한다(적중 수도 항목당 1건으로 집계).
>
> **단, 시각 순연은 B-4(`shift_time.py`)를 쓴다.** 위 매핑 방식은 문서 전역에 걸리므로,
> 보존해야 할 구간(교통편·셔틀 운행표)에 같은 시각 문자열이 있으면 그쪽까지 밀린다.
> 범위를 한정할 수 있고 산술·검산이 자동인 B-4가 정공법이며, 위 매핑은 문서 전체를
> 한 축으로 밀어도 되는 것이 확인된 경우에만 쓴다.
...
## 워크플로우 B-4: 시각 일괄 순연 (발사 연기 등)
일정이 통째로 밀릴 때 문서의 시각 표기를 **산술로** 옮긴다. 자릿수(`9:20` vs `09:20`)와
구분자(`~` `-` `∼`)·접미(`분`·`경`·`까지`)를 원본대로 두고 시각 토큰만 바꾸며,
자정을 넘으면 24시간 순환한다. 소요시간 `(30‘)`·상대시각 `L+20`·`24:00` 이상은 대상이 아니다.

> ⚠️ **전역 순연은 기본적으로 위험하다.** 순연할 구간과 보존할 구간(외부 교통편·셔틀
> 운행표 등)에 **같은 시각 문자열이 동시에 존재**하는 것이 보통이다(실측: 고유 시각
> 156종 중 50종 중복). 범위를 지정하지 않으면 셔틀 시간표까지 함께 밀린다.
> 그래서 `--scope` 미지정 실행은 경고 후 `--yes`를 요구한다.

**반드시 이 순서로 한다: `--dry-run` 확인 → 범위·제외 확정 → 실행 → `--report` 대조표 검토.**
```bash
# ① 드라이런 — 무엇이 바뀔지 먼저 본다(구간 | 맥락 | 현행 | 개정)
python3 scripts/shift_time.py --input 계획.hwpx --shift "+2:45" \
    --scope 'between:"행사 일정".."브리핑 종료"' --dry-run
# ② 실행 + 신구대조표(대상 구간만, 교통편은 구간째로 제외)
python3 scripts/shift_time.py --input 계획.hwpx --output 계획_순연.hwpx --shift "+2:45" \
    --scope 'between:"행사 일정".."브리핑 종료"' --exclude 'after:"교통편 안내"' \
    --report 신구대조.md
```
...
- `--exclude-keyword`(기본: OZ·KTX·아시아나·진에어·항공편·기차편·열차)는 **보조 수단**이다.
  값(`5:08-8:02`)과 라벨(`▪열차 이동`)이 서로 다른 `<hp:t>`에 있는 문서에서는 반드시
  새어 나가므로, 보존은 **구간 지정으로** 한다(`--no-default-keywords`로 해제 가능).
- 변경 전건을 재계산으로 검산하고 시각 외 문자 변경까지 확인한다. 1건이라도 어긋나면
  **파일을 쓰지 않는다.** 노드 수 동일성·구조 검증도 자동 수행한다.
- **표기 교정은 `--pad-hour`로 옵트인**한다. 기본은 원본 자릿수 보존이라 한자리 시
  (`5:08`)가 남고 `validate --kasa`가 "두 자리로"를 경고할 수 있다(순연은 위반을 새로
  만들지 않는다 — 원본에 있던 표기다). `--pad-hour`를 붙이면 **적용 범위 안에서만**
  `9:20`→`09:20`으로 함께 고치므로, 제외한 교통편·셔틀 시간표의 표기는 그대로 남는다.
  순연 없이 표기만 고치려면 `--shift "+0m" --pad-hour`. 표기를 바꿀지는 사용자에게 확인한다.
- 음수 순연은 `--shift "-1:00"`·`--shift=-1:00` 둘 다 된다(argparse 오인을 흡수).
- **드라이런의 「분리 노드」 경고를 반드시 확인한다.** 숫자 일부만 글자 서식이 달라
  한 시각이 두 `<hp:t>`에 걸치면(실측: `’26년 10월 7일, 12` + `:30 예정` — charPr 508/509)
  노드 단위 탐색으로는 잡히지 않아 **조용히 순연에서 빠진다.** 이런 지점은 변경 목록과
  별도로 경고 블록에 뜬다(문단·표 셀·줄바꿈 경계는 결합하지 않으므로 오탐이 적다).
  경고가 뜨면 다음 절차로 처리한다:
  ```bash
  # ① 수정 매핑 생성(드라이런에서 함께 뽑는다 — 문서는 건드리지 않는다)
  python3 scripts/shift_time.py --input 계획.hwpx --shift "+1:30" \
      --scope "section:0" --dry-run --split-fix-map 분리시각.json
  # ② 순연 실행 후, 남은 분리 시각만 재기안으로 치환
  python3 scripts/redraft.py --input 계획_순연.hwpx --map 분리시각.json \
      --output 계획_최종.hwpx --mode exact
  # ③ 재검증
  python3 scripts/validate.py 계획_최종.hwpx --kasa
  ```
  매핑은 **문서 안에서 텍스트가 유일한 노드만** 담는다(같은 텍스트의 `<hp:t>`가 여럿이면
  오치환을 막기 위해 빼고 "수동 확인 필요"로 알린다). 조각을 코드가 **자동으로 병합하지
  않는다** — 서식 경계를 합치면 글자 모양이 바뀌므로 적용은 사람이 판단한다. 값을 고쳐도
  노드는 여전히 쪼개져 있어 경고는 남는다(원본에서 글자 서식을 통일해야 사라진다).
  CI 등에서 누락을 실패로 다루려면 `--fail-on-split`(탐지 시 파일을 쓰지 않고 중단).
...
17-1. **재기안 치환은 원본 기준 동시 적용이다.** 모든 키를 하나의 정규식 alternation으로 컴파일해 원본을 1회 스캔하며 바꾼다(`_compile_rules`). 앞 규칙의 출력이 뒤 규칙의 입력이 되지 않으므로 시각 순연처럼 **새 값이 다른 항목의 기존 값과 겹치는 매핑도 안전**하고, 적중 수도 실제 매칭 횟수만 집계된다. 겹치는 키는 긴 쪽이 먼저 매칭되고(`10:00`보다 `10:00~11:00`), exact 모드는 전체 일치 시 1회만 치환하고 확정한다. 빈 문자열 키는 거부하며(무한 매칭), 키와 값이 같은 규칙은 제외한다. 값에 정규식 특수문자(`\1`·`\g<0>` 등)가 있어도 함수형 replacement로 원문 그대로 삽입된다.
...
23. **시각 순연은 범위를 한정하고 드라이런을 먼저 한다.** 순연 대상과 보존 대상(외부 교통편·셔틀 운행표)에 같은 시각 문자열이 동시에 존재하는 것이 정상이므로, 전역 순연은 위험 동작으로 취급한다(`--scope` 미지정 시 경고 + `--yes` 요구). 보존은 **구간 지정**(`--exclude range:`/`table:`/`after:`)으로 하고 키워드 제외는 보조 수단으로만 쓴다 — 값과 라벨이 서로 다른 `<hp:t>`에 있으면 키워드는 반드시 새어 나간다. 자릿수·구분자·접미 표기는 원본을 보존하며(순연이 표기 위반을 새로 만들지 않는다), 표기 교정이 필요하면 `--pad-hour`로 **적용 범위 안에서만** 옵트인한다 — 문서 전역 정규화는 보존 구간을 오염시키므로 하지 않는다. 변경 전건 재계산 검산이 어긋나면 파일을 쓰지 않는다(`shift_time.py`). 한 시각이 글자 서식 경계로 두 `<hp:t>`에 쪼개져 있으면 노드 단위 탐색으로는 순연되지 않으므로, 그런 지점을 탐지해 경고하고(`--split-fix-map`으로 `redraft --mode exact`용 매핑 생성) **드라이런의 분리 노드 경고를 반드시 확인한 뒤 산출물을 전달한다** — 자동 병합은 글자 모양을 바꾸므로 하지 않는다.
