...
1. **KASA 표기법 lint** — jkf87의 gonmun_lint(공문 표기법 검사)를 KASA 규정에 맞게 이식.
   날짜 표기(`2026. 7. 7.`), 금액·단위, 항목 기호 체계 등을 `validate.py --kasa`에 추가하거나
   별도 `kasa_lint.py`로 분리.
2. **PreToolUse 가드 훅 3종** — Claude Code hooks로 (a) 산출물 finalize 전 `validate --kasa`
   통과 강제, (b) 예시/견본 문서를 실산출물로 오인 제출 차단, (c) 필수 정보(기안자·부서·날짜 등)
   미확보 시 질문 강제. jkf87의 훅 구성 참고.
...
5. **Canine89 unpack 바이트 보존 점검** — unpack→repack 왕복 시 미수정 엔트리의 바이트
   동일성을 검증하는 회귀 테스트 보강(현재는 build/redraft 경로만 테스트).
...
cd kasa-hwpx-skill
python3 -m unittest discover -s tests   # 47개 통과 확인 (Python 3.10+, 외부 의존성 없음)
```
